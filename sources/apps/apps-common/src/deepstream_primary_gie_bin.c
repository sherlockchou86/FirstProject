...
create_primary_gie_bin (NvDsGieConfig *config, NvDsPrimaryGieBin *bin)
{
  ...
  bin->primary_gie =
      gst_element_factory_make (NVDS_ELEM_PGIE, "primary_gie_classifier");
  if (!bin->primary_gie) {
    NVGSTDS_ERR_MSG_V ("Failed to create 'primary_gie_classifier'");
    goto done;
  }
  ...
  if (config->is_unique_id_set)
    g_object_set (G_OBJECT (bin->primary_gie),
        "gie-unique-id", config->unique_id, NULL);
