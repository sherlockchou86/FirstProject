...
all_bbox_generated (AppCtx *appCtx, GstBuffer *buf,
                    NvDsFrameMeta **params1, guint index)
{
  guint num_male = 0;
  guint num_female = 0;
  ...
  for (; *params1; params1++) {
    NvDsFrameMeta *params = *params1;
    guint i;

    for (i = 0; i < params->num_rects; i++) {
      if (params->gie_type == 1) {
        if (params->obj_params[i].class_id >= 0
            && params->obj_params[i].class_id < 128) {
          num_objects[params->obj_params[i].class_id]++;
        }
      }

      /*
       * This is just to demonstrate. It should be modified according to
       * network class types.
       */
      if (appCtx->person_class_id > -1 && params->gie_type == 1
          && params->obj_params[i].class_id == appCtx->person_class_id) {
        if (strstr (params->obj_params[i].text_params.display_text, "Man")) {
          str_replace (params->obj_params[i].text_params.display_text, "Man", "");
          str_replace (params->obj_params[i].text_params.display_text, "Person",
              "Man");
          num_male++;
        } else if (strstr (params->obj_params[i].text_params.display_text,
                "Woman")) {
          str_replace (params->obj_params[i].text_params.display_text, "Woman",
              "");
          str_replace (params->obj_params[i].text_params.display_text, "Person",
              "Woman");
          num_female++;
...
get_source_id_from_coordinates (float x_rel, float y_rel)
{
  ...
  if (source_id >= appCtx[0]->config.num_source_sub_bins)
    source_id = -1;
...
{
  gchar *ins_pos;
  ins_pos = params->obj_params[0].text_params.display_text;

  if (source_ids[index] == -1)
    return TRUE;

  g_snprintf (ins_pos, MAX_BBOX_TEXT_SIZE, "Source: %s",
              appCtx->config.multi_source_config[source_ids[index]].uri);
  ins_pos[MAX_BBOX_TEXT_SIZE - 1] = '\0';


  params->obj_params[0].text_params.y_offset = 20;
  params->obj_params[0].text_params.x_offset = 20;
  params->obj_params[0].text_params.font_params.font_color = (NvOSD_ColorParams) {1, 1, 1, 1};
  params->obj_params[0].text_params.font_params.font_size =
    appCtx->config.osd_config.text_size * 1.5;
  params->obj_params[0].text_params.font_params.font_name = "Arial";
  params->obj_params[0].text_params.set_bg_clr = 1;
  params->obj_params[0].text_params.text_bg_clr = (NvOSD_ColorParams) {0, 0, 0, 0.5};
  params->num_strings = 1;
  return TRUE;
}
